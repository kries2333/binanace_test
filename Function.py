...
def dingding_report_candle_data(symbol_candle_data, symbol_config, robot_id_secret):
    content = ''
    symbol_info_str = ''
    for symbol in symbol_config.keys():
        for x, y in symbol_candle_data[symbol].iterrows():
            symbol_info_str += ['\n\n' + str(x) + '\n' + y.to_string()]

    content += '# =====当前行情' + ''.join(symbol_info_str) + '\n\n'
